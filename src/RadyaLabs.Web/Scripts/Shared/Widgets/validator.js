Validator = {
    init: function () {
        ...
        $('.datalist-value').on('change', function () {
            var validator = $(this).parents('form').validate();
            var control = new MvcDatalist(this).control;
        ...
        $('form').on('invalid-form', function (e, validator) {
            $(this).find('.datalist-values').each(function (i, values) {
                [].forEach.call(values.children, function (value) {
                    var control = new MvcDatalist(values).control;
        ...
        $('.datalist-value.input-validation-error').each(function (i, element) {
            new MvcDatalist(element).control.classList.add('input-validation-error');
        });

        var currentIgnore = $.validator.defaults.ignore;
        $.validator.setDefaults({
            ignore: function () {
                return $(this).is(currentIgnore) && !this.classList.contains('datalist-value');
            }
        });
